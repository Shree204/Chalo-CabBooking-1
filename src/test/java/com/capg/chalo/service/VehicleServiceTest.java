...
class VehicleServiceTest {
	...
	void testAddVehicle() {
		...
		assertNotNull(vehicleRepository.findById(1L).get());
	...
	void testGetVehicle() throws VehicleNotFoundException {
		Vehicle vehicle = new Vehicle();
		vehicle = vehicleService.showVehicleById(1L);
	...
	void testUpdateVehicle() {
		...
		vehicleRepository.save(vehicle);
		assertNotNull("123ab", vehicleRepository.findById(1L).get().getRegistrationNo());
		assertNotNull("car", vehicleRepository.findById(1L).get().getVehicleType());
	}
