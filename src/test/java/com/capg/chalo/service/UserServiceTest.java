...
class UserServiceTest {
	...
	@Test
	void addUserTest() throws UserAlreadyExistException {
		Mockito.when(userRepository.save(user)).thenReturn(user);
		assertThat(userService.addUser(user)).isEqualTo(user);
	}

	@Test
	void getAllUsersTest() throws EmptyUserListException {
	...
	@Test
	void getUserByIdTest() throws CustomException, UserNotFoundException {
		Mockito.when(userRepository.findById(1l)).thenReturn(Optional.of(user));
		assertThat(userService.showUserById(1l)).isEqualTo(user);
	}
